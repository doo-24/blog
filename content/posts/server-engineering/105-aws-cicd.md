...
프로덕션 환경에서는 Blue/Green이 권장된다. In-Place 방식은 배포 중 구버전을 중단하기 때문에 그 순간 서비스가 일시 중단되고, 문제가 생기면 다시 구버전을 설치해야 해서 복구 시간이 길다. Blue/Green은 신버전 그룹을 완전히 준비한 뒤 로드밸런서 트래픽만 전환하므로 다운타임이 없고, 롤백도 트래픽을 원래 그룹으로 되돌리면 즉시 완료된다.
