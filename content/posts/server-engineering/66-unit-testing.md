...
Stub   → "협력자가 X를 반환하면 내 코드가 올바른가?" (상태 검증)
         실제 구현 없음, 미리 정해진 값만 반환

Mock   → "이 메서드가 정확히 호출됐는가?" (행동 검증)
         호출 횟수·인자 검증, verify() 사용

Spy    → 실제 객체를 감싸되 일부 호출만 추적/오버라이드
         레거시 코드, 실제 동작 + 호출 기록 필요 시

Fake   → 실제 동작하는 경량 구현체
         예: InMemoryRepository, 인메모리 이메일 서버
         프로덕션에는 쓰지 않는 간단한 대체 구현
```
...
Fake는 실제로 동작하지만 프로덕션에서는 사용하기 부적합한 단순화된 구현체다. 영속성이 없고, 분산 환경에서 공유되지 않으며, 프로덕션 수준의 성능이나 장애 처리를 갖추지 않기 때문이다. 테스트에서는 속도와 단순성이 목적이므로 이런 제약이 오히려 장점이 된다.
