...
AWS RDS, S3, EBS는 기본적으로 AES-256 암호화를 제공한다. 데이터베이스 인스턴스를 생성할 때 암호화를 활성화하면 스토리지 계층 전체가 암호화된다. 이 방식은 물리적 디스크 탈취 시나리오를 방어하지만, DB 접근 권한이 있는 공격자는 여전히 평문 데이터를 읽을 수 있다. 스토리지 암호화는 "데이터센터 직원이 디스크를 들고 나간다"는 위협 모델을 방어하는 것이지, DB 계정 탈취를 막는 것이 아니다.
...
**내부 서비스 간 통신도 TLS를 적용해야 한다.** 공격자가 내부 네트워크에 침투한 경우, 암호화되지 않은 내부 통신을 도청해 민감 데이터를 탈취할 수 있기 때문이다. 서비스 메시(Istio, Linkerd)를 사용하는 Kubernetes 환경에서는 mTLS(상호 TLS)로 서비스 간 인증까지 처리할 수 있다.
