...
p95와 p99의 차이가 크면 특정 요청 유형이나 특정 데이터 조건에서 느린 쿼리가 존재한다는 신호다. 평균 응답 시간이 빠르더라도 p99가 수 초라면, 전체 사용자의 1%가 매우 나쁜 경험을 하고 있다는 의미이고 이는 실제 서비스에서는 수만 명에 해당할 수 있다.
...
```text
k6 (부하 생성) ──메트릭 전송──► InfluxDB (저장) ──► Grafana (시각화)
     │                                                      │
     │  TPS, p95, p99, 에러율                         대시보드
     └──────────────────────────────────────────────► 실시간 확인
```
