...
런칭 전날 QA를 통과한 서버가, 오픈 당일 유저가 몰리자 3분 만에 응답을 멈추는 장면은 낯설지 않다. 부하 테스트는 그 장면을 미리 재현해서 서버의 한계를 사전에 파악하는 실험이다. 단순히 "버티는지 확인"하는 게 아니라, 병목이 어디에 있는지, 어느 지점에서 성능이 꺾이는지, 복구는 되는지를 측정하는 과학적 과정이다.
