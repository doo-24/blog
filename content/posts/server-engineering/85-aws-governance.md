...
`NotAction`은 나열한 서비스를 **제외한** 나머지 모두에 적용하는 역 선택 방식이다. IAM, Route53, CloudFront처럼 리전에 종속되지 않는 글로벌 서비스는 리전 제한 대상에서 빠져야 하는데, 이를 허용 목록으로 일일이 나열하면 수백 개 서비스 전체를 명시해야 한다. `NotAction`으로 예외 목록만 선언하면 정책을 훨씬 간결하게 유지할 수 있다.
