...
인터넷 요청
  |
  ▼
[WAF Web ACL]  ← 규칙 평가 (우선순위 순)
  ├── IP 허용 목록 (Priority 0)  →  Allow (관리자 IP)
  ├── Rate-based 규칙 (Priority 3)  →  Block (5분 2000회 초과)
  ├── 관리형 규칙 (Priority 10)  →  Block (OWASP Top 10 등)
  └── Default Action  →  Allow (위 규칙 미해당)
  |
  ▼
...
WAF는 HTTP 레이어 공격을, Shield는 볼륨 DDoS를, GuardDuty는 계정·인프라 수준 이상을, Config Rules는 구성 준수를 각각 담당한다. 각 서비스가 보는 데이터가 다르기 때문에 하나로 대체할 수 없다. WAF는 HTTP 요청 내용을 분석하고, Shield는 네트워크 패킷 볼륨을 감지하며, GuardDuty는 API 호출 패턴과 계정 행동을 학습한다. 네 계층이 모두 작동할 때 비로소 방어 깊이(Defense in Depth)가 완성된다.
