...
CloudWatch Logs는 로그 데이터를 **로그 그룹(Log Group)** > **로그 스트림(Log Stream)** 계층으로 관리한다. 로그 그룹은 애플리케이션 단위의 폴더, 로그 스트림은 그 안의 개별 파일에 해당한다. 컨테이너 환경에서는 인스턴스 또는 Pod 하나가 로그 스트림 하나를 생성한다. 애플리케이션 하나당 로그 그룹 하나를 만드는 것이 일반적인 관례다.
