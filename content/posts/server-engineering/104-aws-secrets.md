...
KMS는 대용량 데이터를 직접 암호화하지 않는다. 대신 봉투 암호화 방식을 사용한다. 비유하자면, 귀중품을 직접 금고에 넣는 대신 작은 자물쇠(DEK)로 잠근 뒤 그 자물쇠 열쇠만 금고(KMS)에 보관하는 방식이다. 금고는 열쇠만 관리하면 되므로 부담이 적고, 귀중품이 아무리 커도 자물쇠 교체만으로 보안을 유지할 수 있다.
