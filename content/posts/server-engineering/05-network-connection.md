---
title: "[OS와 네트워크] 5편 — 네트워크와 커넥션: 서버가 요청을 받고 응답을 돌려주기까지"
...
하지만 서버 엔지니어에게는 각 단계의 **커널 레벨 동작**이 중요하다. TCP 연결 하나가 만들어지는 과정에서 커널은 어떤 자료구조를 사용하고, 소켓 버퍼는 어떻게 관리되며, TIME_WAIT 상태는 왜 서버를 괴롭히는가?

이 편에서는 네트워크 통신의 기초를 커널의 관점에서 깊이 다룬다.
...
**SYN Cookie**는 SYN Queue 없이 핸드셰이크를 완료하는 기법이다. 연결 정보를 서버 메모리에 저장하는 대신, SYN+ACK의 초기 시퀀스 번호(ISN) 안에 암호화해서 담는다. 클라이언트가 ACK를 보내면 그 시퀀스 번호에서 연결 정보를 역산하므로 Queue 없이 상태를 복원할 수 있다.
