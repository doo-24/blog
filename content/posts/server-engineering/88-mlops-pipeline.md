...
소프트웨어 개발에 CI/CD가 있다면 ML에는 MLOps가 있다. 모델을 한 번 학습하고 배포하면 끝나는 것이 아니다.

데이터는 변하고, 세상은 변하고, 모델의 예측은 조용히 틀려간다.

MLOps 파이프라인은 이 사이클 전체를 자동화한다. 데이터 수집부터 배포, 모니터링, 재학습까지.
...
전처리 로직은 반드시 재현 가능해야 한다. 학습 때 쓴 전처리와 서빙 때 쓰는 전처리가 달라지는 순간 예측 품질이 무너진다. 이 현상을 **Train-Serve Skew**라 하며, ML 시스템 장애의 가장 흔한 원인 중 하나다. 전처리기 객체를 학습 시 저장하고 서빙 시 그대로 로드하면 이 불일치를 원천 차단할 수 있다.
