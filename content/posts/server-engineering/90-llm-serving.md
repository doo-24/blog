...
대규모 언어 모델(LLM)은 기존 ML 모델과 추론 특성이 근본적으로 다르다. 이미지 분류 모델은 입력을 받아 한 번에 결과를 내지만, GPT 계열 모델은 토큰을 하나씩 순차 생성한다. 이 차이가 서빙 인프라 설계 전반을 바꾼다. 메모리 관리, 배칭 전략, 스트리밍 응답, 비용 구조 모두 LLM 특유의 방식이 필요하다.
