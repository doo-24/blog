...
컨테이너를 프로덕션에서 운영하려면 오케스트레이션이 필요하다. AWS에서는 ECS와 EKS, 두 가지 선택지가 있다.

이 글은 두 서비스의 아키텍처 차이부터 실전 배포 파이프라인까지 다룬다.
...
오래된 이미지를 방치하면 스토리지 비용이 쌓인다. CI/CD 파이프라인이 하루에도 수십 번 이미지를 빌드하는 환경에서는 untagged 이미지가 빠르게 누적되어 수백 GB에 달하는 비용이 발생할 수 있다.
