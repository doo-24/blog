...
"타임스탬프가 같거나 역전된 두 이벤트가 실제로 어떤 관계인지" 알 수 없다. A의 타임스탬프가 B보다 크다고 해서 A가 반드시 B 이후에 발생한 것은 아니다. 두 이벤트가 서로 영향을 주지 않고 독립적으로 발생했는지(동시 발생), 아니면 인과 관계가 있는지를 Lamport Clock만으로는 구분할 수 없다.
...
이 정리가 의미하는 것은 완벽한 합의가 이론적으로 불가능하다는 게 아니라, 장애와 비동기성을 동시에 다루는 알고리즘은 반드시 어딘가에서 타협해야 한다는 것이다. 실용적인 합의 알고리즘은 이 한계를 "충분히 오랜 안정적 기간"을 가정함으로써 우회한다.
