...
CPU 코어 하나는 한 번에 하나의 스레드만 실행할 수 있다. 서버에는 수백 개의 스레드가 존재한다.

Tomcat 스레드 200개, GC 스레드 4개, Kafka 컨슈머 스레드 10개, OS 자체 스레드 수십 개. 이 모든 스레드가 CPU를 원한다.
...
단순히 실행 시간을 비교하지 않고 vruntime을 도입하는 이유는 **우선순위를 자연스럽게 반영**하기 위해서다. 우선순위가 높은 프로세스는 같은 시간을 실행해도 vruntime이 천천히 늘어나므로, 항상 "덜 실행된 것처럼" 보여서 더 자주 CPU를 받게 된다.
