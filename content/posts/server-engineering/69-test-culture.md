...
테스트를 먼저 작성하면 인터페이스를 사용자의 관점에서 설계하게 되고, 자연스럽게 테스트하기 쉬운 구조가 만들어진다. 코드를 먼저 작성하면 이미 만들어진 구조에 맞춰 테스트를 끼워 맞추게 되어 의존성이 복잡하게 얽히기 쉽다. 한 번에 큰 기능을 구현하려 하면 TDD가 오히려 방해가 된다.
