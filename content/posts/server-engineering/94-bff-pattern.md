...
하나의 백엔드 API를 웹, 모바일, 서드파티가 공유하면 어느 쪽도 만족시키지 못하는 API가 된다.

BFF는 클라이언트별로 최적화된 백엔드 계층을 두는 패턴이다. 이 글은 BFF의 필요성부터 구현, 테스트, 장애 격리까지 다룬다.
...
모바일 BFF가 다운되면 모바일 앱 전체가 동작하지 않는다. 마이크로서비스 간에는 장애가 격리되지만, BFF는 그 위에 있는 단일 진입점이기 때문에 장애 범위가 클라이언트 전체로 확대된다.
...
서킷 브레이커가 열리면 Recommendation Service로 요청을 보내지 않는다. 바로 Fallback을 반환해서 BFF 응답이 빠르게 유지된다. 반복 실패하는 서비스를 계속 호출하면 BFF 스레드가 타임아웃 대기로 묶여 전체 응답이 지연되는데, 서킷 브레이커는 이 연쇄 지연을 차단하는 역할을 한다.
