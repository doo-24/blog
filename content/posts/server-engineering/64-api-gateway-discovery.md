...
클라이언트                  API Gateway                    내부 서비스
   │                            │
   │   GET /api/users/**        │  인증/인가 (JWT 검증)
   ├───────────────────────────►│  Rate Limiting
   │                            │  로깅/추적                ┌─ User Service
   │   POST /api/orders/**      ├──────────────────────────►│
   ├───────────────────────────►│  라우팅                   ├─ Order Service
   │                            ├──────────────────────────►│
   │   GET /api/products/**     │  집계 (Aggregation)       ├─ Product Service
   └───────────────────────────►├──────────────────────────►│
                                │  ← 여러 응답 병합          └─ Payment Service
                            단일 응답 반환
```
...
Client ──조회──► Service Registry
  ...
  │◄──────────────
  │
  ├──요청──► Service Instance A
  ├──요청──► Service Instance B  ← 클라이언트가 직접 LB
  └──요청──► Service Instance C
```
...
```text
Client ──요청──► Load Balancer ──조회──► Service Registry
                     │                   (Consul/K8s DNS)
                     │  인스턴스 선택
                     ├──라우팅──► Service Instance A
                     └──라우팅──► Service Instance B
```
