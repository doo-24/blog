...
100만 개 레코드를 가진 BST의 높이는 약 20이다. 즉 탐색 하나에 최대 20번의 디스크 I/O가 발생한다. SSD라도 랜덤 I/O 하나에 0.1ms라면, 20번이면 2ms다. 초당 500개 쿼리도 처리 못 한다.
...
UUID는 무작위 순서이기 때문에 삽입 위치가 기존 리프 어디에나 될 수 있다. 이미 가득 찬 리프 중간에 삽입해야 하는 경우가 빈번하게 발생해 분할이 폭발적으로 증가한다.
