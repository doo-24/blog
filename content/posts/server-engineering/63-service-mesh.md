...
추방 시간은 연속 추방될수록 지수적으로 증가한다 (`baseEjectionTime * 추방 횟수`). 이는 일시적인 장애가 회복될 시간을 주면서, 반복적으로 문제가 되는 엔드포인트는 더 오래 격리해 클러스터 전체를 보호하기 위함이다. `maxEjectionPercent`를 100으로 설정하면 모든 엔드포인트가 추방되어 서비스 전체가 다운될 수 있으므로 반드시 제한을 둬야 한다.
