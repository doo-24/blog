...
VPC CIDR은 나중에 변경이 매우 어렵다. 처음부터 충분한 IP 공간을 확보해야 한다.
...
NACL은 Stateless이므로 임시 포트(1024-65535) 인바운드 규칙을 반드시 추가해야 한다. 이 규칙이 없으면 외부에서 들어온 HTTP 요청의 응답이 클라이언트에게 전달되지 않는다.
