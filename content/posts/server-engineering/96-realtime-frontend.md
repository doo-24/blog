...
Jitter는 중요하다. 서버가 재시작된 직후 수천 개의 클라이언트가 동시에 재연결을 시도하면 서버가 다시 과부하에 걸린다.
...
SSE의 인증 한계를 해결하는 더 깔끔한 방법은 쿠키 기반 세션 인증을 사용하는 것이다. HttpOnly 쿠키는 EventSource 요청에 자동으로 포함된다.
