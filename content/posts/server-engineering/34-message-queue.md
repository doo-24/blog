...
```text
[동기 방식 — 모든 서비스가 직렬로 블로킹]

주문 서비스 ──→ 재고 서비스(50ms) ──→ 결제 서비스(200ms) ──→ 알림 서비스(150ms) ──→ 포인트(30ms)
              (실패 시 전체 롤백)     (느리면 전체 대기)

총 응답 시간: 430ms, 어느 하나 장애 = 전체 중단

[메시지 큐 방식 — 발행 후 즉시 반환]

주문 서비스 ──→ [Message Queue] ──→ 재고 서비스 (비동기)
     |                     └──────→ 결제 서비스 (비동기)
     ↓ (즉시 응답)                  └──────→ 알림 서비스 (비동기)
  사용자                            └──────→ 포인트 서비스 (비동기)

응답 시간: ~10ms, 각 서비스 독립 장애 격리
```
