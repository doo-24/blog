...
단순히 `Thread.sleep()`으로 대기하는 것은 느리고 불안정하다. 처리가 빠르게 끝나도 sleep 시간만큼 무조건 기다려야 하고, 처리가 느려지면 오히려 실패한다. Awaitility 라이브러리는 조건이 충족될 때까지 짧은 간격으로 반복 확인하므로 처리가 빠를 때는 즉시 통과하고 느릴 때는 최대 대기 시간까지 유연하게 기다린다.
