...
함수 호출마다 **스택 프레임**이 쌓인다. 지역 변수, 함수 인자, 리턴 주소가 저장된다.

함수가 리턴하면 스택 프레임이 제거된다. LIFO(Last In, First Out) 구조다.
...
물리 메모리가 부족해지면, 커널은 오래 접근하지 않은 메모리 페이지를 디스크(스왑 영역)로 내보내고, 필요할 때 다시 물리 메모리로 읽어온다. 스왑은 "메모리를 디스크로 확장하는" 개념이지만, 디스크는 메모리보다 수천~수만 배 느리기 때문에 성능 대가가 크다. 스왑이 활발해진다는 것은 시스템이 이미 용량 한계를 넘어섰다는 신호로 봐야 한다.
...
물리 메모리와 스왑이 모두 소진되어 더 이상 페이지를 할당할 수 없으면, 커널의 OOM(Out of Memory) Killer가 동작한다. 아무것도 하지 않으면 커널 자체가 멈추므로, 가장 적은 피해로 메모리를 확보하기 위해 프로세스 하나를 선택해 강제 종료하는 것이다.
...
이것이 서버를 재부팅하면 처음에 느린 이유다. 페이지 캐시가 비어있어서 모든 파일 접근이 디스크를 거친다.

시간이 지나면 자주 사용하는 파일이 캐시에 올라와서 빨라진다. 이를 **"캐시 워밍"**이라 한다.
