...
시간 →                        시간 →
v1 v1 v1  (다운타임)  v2 v2   v1 v1 v1 → v2 v1 v1 → v2 v2 v1 → v2 v2 v2
           ↑                              (점진적 교체, 무중단)
        서비스 중단

[카나리]                       [블루-그린]
시간 →                        시간 →
v1 v1 v1 v1 v1 v2             Blue(v1): v1 v1 v1  ─► 트래픽 전환 ─► (제거)
          └─ 5% 트래픽만       Green(v2): v2 v2 v2 ─► 전체 트래픽
             검증 후 전환                              (즉시 롤백 가능)
```
...
다운타임이 발생하지만, 데이터베이스 마이그레이션처럼 **구버전과 신버전이 동시에 실행되면 안 되는 경우**에 적합하다. 구버전과 신버전이 동시에 실행되면 스키마 변경이나 싱글턴 잠금이 충돌해 데이터 무결성이 깨질 수 있기 때문이다.
...
`averageUtilization: 60`은 전체 파드의 평균 CPU 사용률이 60%가 되도록 유지한다는 의미다. 100%에 가깝게 설정하면 스케일 업 트리거 시점에 이미 과부하가 걸려 지연이 누적되고, 너무 낮게 설정하면 비용이 낭비된다. 60~70% 목표는 급작스러운 트래픽 증가에 새 파드가 준비되는 시간(약 30~60초)을 확보하기 위한 버퍼다.
