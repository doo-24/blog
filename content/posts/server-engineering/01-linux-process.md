...
6. [시그널 — 프로세스에 보내는 메시지](#6-시그널--프로세스에-보내는-메시지)
7. [/proc 파일시스템 — 커널이 펼쳐 보이는 정보](#7-proc-파일시스템--커널이-펼쳐-보이는-정보)
8. [strace — 시스템 콜을 엿보다](#8-strace--시스템-콜을-엿보다)
9. [lsof — 열린 파일과 연결 상태 파악](#9-lsof--열린-파일과-연결-상태-파악)
10. [프로세스 우선순위 — nice와 renice](#10-프로세스-우선순위--nice와-renice)
...
이 분리는 CPU의 보호 링(Protection Ring) 메커니즘으로 강제된다. x86 CPU는 Ring 0(커널)부터 Ring 3(유저)까지 4개의 권한 레벨을 제공하는데, 리눅스는 Ring 0과 Ring 3만 사용한다.

유저 공간의 코드가 커널 메모리에 접근하려 하면 CPU가 예외(exception)를 발생시키고, 커널이 해당 프로세스를 종료한다.
