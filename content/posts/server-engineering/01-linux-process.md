...
서버에 SSH로 접속해서 `java -jar app.jar`을 실행하는 순간, OS 안에서는 수십 가지 일이 벌어진다.

메모리가 할당되고, CPU 스케줄링 큐에 등록되고, 파일 디스크립터 테이블이 만들어지고, 네트워크 소켓이 열린다.
...
1. [커널 공간과 유저 공간](#1-커널-공간과-유저-공간)
2. [프로세스의 정체](#2-프로세스의-정체)
3. [프로세스 vs 스레드](#3-프로세스-vs-스레드)
4. [fork와 exec — 프로세스의 탄생](#4-fork와-exec--프로세스의-탄생)
5. [프로세스 상태와 좀비](#5-프로세스-상태와-좀비)
6. [파일 디스크립터 — 모든 것은 파일이다](#6-파일-디스크립터--모든-것은-파일이다)
7. [시그널 — 프로세스에 보내는 메시지](#7-시그널--프로세스에-보내는-메시지)
8. [/proc 파일시스템 — 커널이 펼쳐 보이는 정보](#8-proc-파일시스템--커널이-펼쳐-보이는-정보)
9. [strace — 시스템 콜을 엿보다](#9-strace--시스템-콜을-엿보다)
10. [lsof — 열린 파일과 연결 상태 파악](#10-lsof--열린-파일과-연결-상태-파악)
11. [프로세스 우선순위 — nice와 renice](#11-프로세스-우선순위--nice와-renice)
...
이 분리는 CPU의 보호 링(Protection Ring) 메커니즘으로 강제된다. x86 CPU는 Ring 0(최고 권한)부터 Ring 3(최저 권한)까지 4개의 권한 레벨을 하드웨어로 제공한다. 리눅스는 Ring 1, 2는 사용하지 않고 Ring 0(커널 모드)과 Ring 3(유저 모드)만 사용한다. 단순하게 "커널이냐 아니냐" 두 가지만 구분하는 것으로 충분하기 때문이다.

유저 공간의 코드가 커널 메모리에 접근하려 하면 CPU가 예외(exception)를 발생시키고, 커널이 해당 프로세스를 종료한다. 흔히 보는 "Segmentation fault"가 이것이다.
...
이것이 Tomcat의 스레드 풀 크기를 CPU 코어 수와 맞춰 설정하는 이유다. 200개 스레드가 4코어 CPU에서 돌면, 196개는 항상 대기 중이고, 전환 비용만 쌓인다.

다만 I/O 대기가 많은 웹 서버에서는 스레드 수가 코어 수보다 많아도 괜찮다. I/O 대기 중인 스레드는 CPU를 쓰지 않기 때문이다.

**그럼 몇 개가 적당한가?**

- **CPU 바운드 작업** (이미지 처리, 암호화 등): 코어 수 + 1 ~ 2 정도. 더 늘려봐야 전환 비용만 증가한다.
- **I/O 바운드 작업** (DB 조회, 외부 API 호출 등): Little's Law로 추정할 수 있다. `스레드 수 = 목표 TPS × 평균 응답시간(초)`. 예를 들어 목표 TPS 500, 평균 응답 200ms이면 `500 × 0.2 = 100`개 정도가 출발점이다.

실제로는 부하 테스트를 통해 TPS와 응답시간이 안정되는 지점을 찾는다. Tomcat 기본값(200)은 대부분의 서비스에서 과하거나 부족하다.
...
**Copy-on-Write(COW)**: 실제로 메모리를 전부 복사하면 느리다. 리눅스는 fork 직후 부모와 자식이 같은 물리 페이지를 공유하게 한다.

어느 한쪽이 페이지를 **수정하는 순간**에만 해당 페이지를 복사한다. 읽기만 하면 복사가 일어나지 않으므로, fork 자체는 매우 빠르다.
...
> **Dockerfile에서 CMD/ENTRYPOINT는 항상 exec 형태로 작성한다**
>
> `CMD java -jar app.jar` (shell 형태) → sh가 PID 1, Java는 자식 프로세스 → SIGTERM 전달 안 됨
> `CMD ["java", "-jar", "app.jar"]` (exec 형태) → Java가 PID 1 → SIGTERM 직접 수신
>
> 이 차이 하나가 배포 시 진행 중인 요청이 정상 완료되는지, 강제 끊기는지를 결정한다.
...
D 상태는 SIGKILL로도 죽일 수 없기 때문에 서버가 응답 불가 상태에 빠지는 가장 위험한 상황 중 하나다. 원인은 대부분 디스크 I/O가 극도로 느리거나, NFS 마운트가 응답하지 않는 경우다.

```bash
# D 상태 프로세스 확인
$ ps aux | awk '$8 ~ /D/'
USER       PID %CPU %MEM    VSZ   RSS TTY  STAT COMMAND
app      12345  0.0  0.1  12345  1234 ?    D    java -jar app.jar

# 어느 디스크에서 병목이 생기는지
$ iostat -x 1
Device    %util  await
xvda      99.8   450.3   ← %util 100%, await 450ms — 완전 포화

# 어떤 프로세스가 I/O를 많이 하는지
$ iotop -o         # I/O 발생 중인 프로세스만 표시
```

D 상태가 NFS 문제인 경우 `umount -f -l`로 강제 마운트 해제 후 재마운트를 시도한다. 디스크 과부하인 경우 원인 프로세스(대량 로그, 배치 작업 등)를 중단하거나 `ionice`로 우선순위를 낮춘다.
...
> **컨테이너에서 좀비가 쌓이면 tini를 의심하라**
>
> 컨테이너의 PID 1이 Java 앱이면, Java는 wait()를 구현하지 않으므로 자식 프로세스가 죽을 때 좀비로 남는다. 좀비가 PID를 고갈시키면 새 프로세스를 만들 수 없게 된다. `tini`(또는 `dumb-init`)을 PID 1으로 두는 것이 표준 해결책이다.

---

## 6. 파일 디스크립터 — 모든 것은 파일이다

### "모든 것은 파일이다"

리눅스의 핵심 설계 철학이다. 일반 파일만 "파일"이 아니다. 네트워크 소켓, 파이프, 디바이스, 심지어 `/proc` 아래의 프로세스 정보까지 전부 "파일"로 추상화된다.

이 추상화가 없었다면 파일을 읽는 API, 소켓을 읽는 API, 파이프를 읽는 API가 각각 달라야 했을 것이다. "모든 것은 파일이다" 덕분에 `read()` / `write()` 하나로 전부를 다룰 수 있고, 프로그램이 입출력 대상을 런타임에 교체할 수도 있다.

프로세스는 이 모든 리소스를 **파일 디스크립터(File Descriptor, fd)** 라는 정수 번호로 접근한다. 파일을 열든, 소켓을 만들든, 파이프를 생성하든, 커널은 항상 fd 번호를 돌려준다. 이후 모든 읽기/쓰기는 이 번호를 통해 이루어진다.

### 기본 fd 세 개

모든 프로세스는 생성되는 순간 세 개의 fd를 자동으로 갖는다.

| fd | 이름 | 의미 |
|----|------|------|
| 0 | stdin | 표준 입력 (키보드, 파이프 입력) |
| 1 | stdout | 표준 출력 (화면, 파이프 출력) |
| 2 | stderr | 표준 에러 (에러 메시지 출력) |

이후 파일을 열거나 소켓을 생성할 때마다 3, 4, 5... 순서로 번호가 할당된다.

Spring Boot 앱이 실행되면 어떤 fd들이 생기는지 `/proc/<pid>/fd`에서 직접 확인할 수 있다.

```bash
$ ls -la /proc/12345/fd/
lr-x------ 0 -> /dev/null
l-wx------ 1 -> pipe:[12345]       ← stdout (로그 파이프)
l-wx------ 2 -> pipe:[12346]       ← stderr
lrwx------ 10 -> socket:[23456]    ← 8080 포트 리스닝 소켓
lrwx------ 11 -> socket:[23457]    ← 클라이언트 연결 소켓
lrwx------ 15 -> socket:[34567]    ← DB 커넥션 소켓
lr-x------ 20 -> /app/config.yml   ← 설정 파일
```

소켓도 fd다. DB 커넥션도, HTTP 클라이언트 연결도 전부 fd를 하나씩 소비한다.

### "Too many open files"

서버 운영에서 자주 마주치는 에러다. 원인은 단순하다. 프로세스가 열 수 있는 fd 수에 **운영체제 레벨의 제한**이 있는데, 그 한계를 넘긴 것이다.

동시 접속 1000개짜리 서버를 예로 들면:

```
클라이언트 소켓:   1,000개  (연결마다 소켓 fd 하나)
DB 커넥션 풀:        50개  (풀 크기만큼 fd 점유)
Redis 커넥션:        10개
로그 파일, 설정 파일:  수십 개
─────────────────────────
합계:             ~1,100개
...
```bash
$ ulimit -n        # 현재 소프트 리밋 (보통 1024)
1024
$ ulimit -Hn       # 하드 리밋
65536
```

기본값 1024는 프로덕션에서 거의 항상 부족하다. 동시 접속이 수백 개만 돼도 금방 한계에 닿는다.

> **프로덕션 서버 필수 설정**
>
> `ulimit -n`이 1024이면 즉시 변경해야 한다. 가장 확실한 방법은 systemd 서비스 파일에 직접 명시하는 것이다.
> ```
> [Service]
> LimitNOFILE=65536
> ```
> `/etc/security/limits.conf`로 변경해도 되지만, systemd 서비스는 PAM 설정을 무시하는 경우가 있어 서비스 파일이 더 확실하다.

### fd 누수 진단

fd 수가 시간이 지나도 계속 증가하고 줄지 않는다면 **fd 누수**다. HTTP 클라이언트나 DB 커넥션을 열고 닫지 않는 코드가 원인인 경우가 많다.
...
# fd 유형별 집계 (lsof와 함께 쓰면 더 상세)
$ ls -la /proc/12345/fd | grep socket | wc -l   # 소켓 수
$ ls -la /proc/12345/fd | grep pipe | wc -l     # 파이프 수

# 5초 간격으로 fd 수 추이 관찰
$ watch -n 5 'ls /proc/12345/fd | wc -l'
```

fd 수가 계속 우상향하면 try-with-resources, 커넥션 풀 반환 로직을 점검한다.
...
| 시그널 | 번호 | 보내는 방법 | 의미 | 잡을 수 있는가 |
|--------|------|------------|------|----------------|
| SIGTERM | 15 | `kill <pid>` | 정중한 종료 요청 | O |
| SIGKILL | 9 | `kill -9 <pid>` | 강제 즉시 종료 | X (커널 직접 처리) |
| SIGINT | 2 | Ctrl+C | 인터럽트 | O |
| SIGHUP | 1 | `kill -HUP <pid>` | 설정 재로드 관례 | O |
| SIGSTOP | 19 | `kill -STOP <pid>` | 일시 정지 | X |
| SIGCONT | 18 | `kill -CONT <pid>` | 정지된 프로세스 재개 | O |
| SIGCHLD | 17 | (커널 자동 전송) | 자식 프로세스 종료 알림 | O |
...
> **SIGKILL은 최후의 수단이다**
>
> `kill -9`는 커널이 직접 처리하므로 프로세스가 절대 잡을 수 없다. DB 커넥션 정리도, 진행 중인 요청 완료도 없이 즉시 제거된다. 항상 SIGTERM을 먼저 보내고, 타임아웃 이후에만 SIGKILL을 쓴다.
>
> exit code로 원인을 빠르게 좁힐 수 있다: **137** = SIGKILL (OOM 또는 강제 종료), **143** = SIGTERM (정상 종료 요청), **1** = 애플리케이션 내부 에러
...
> **Kubernetes 환경에서 타임아웃 설정 주의**
>
> Spring의 `timeout-per-shutdown-phase`는 반드시 Kubernetes의 `terminationGracePeriodSeconds`보다 **짧게** 설정해야 한다. K8s는 SIGTERM 전송 후 `terminationGracePeriodSeconds`(기본 30초)가 지나면 SIGKILL을 보낸다. Spring 타임아웃이 K8s 타임아웃과 같거나 길면, 요청 처리를 마치기 전에 SIGKILL이 날아온다.
>
> 권장 설정: `timeout-per-shutdown-phase: 25s` + `terminationGracePeriodSeconds: 35`

---

## 8. /proc 파일시스템 — 커널이 펼쳐 보이는 정보
...
MemFree:          512000 kB    ← 완전히 비어 있는 메모리
MemAvailable:    3276800 kB    ← 실제로 사용 가능한 메모리
Buffers:          131072 kB
Cached:          2621440 kB

> **메모리 부족 판단 시 MemFree가 아닌 MemAvailable을 봐야 한다**
>
> 리눅스는 남는 메모리를 디스크 캐시로 적극 활용한다. 그래서 `MemFree`는 항상 작게 표시되는데, 이걸 보고 메모리가 부족하다고 오판하는 경우가 많다. `MemAvailable`은 캐시 중 회수 가능한 부분까지 포함한 값으로, 실제로 새 프로세스가 쓸 수 있는 메모리다. `free -h` 명령의 `available` 컬럼이 이 값이다.
...
| 파일 디스크립터 | 소켓/파이프/파일 모두 fd, ulimit 기본값 1024는 프로덕션에서 부족 |
