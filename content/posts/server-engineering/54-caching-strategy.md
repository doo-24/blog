...
모든 캐시가 동시에 만료되면 대규모 캐시 미스가 한꺼번에 발생한다. 이를 **Thunder Herd** 또는 **캐시 스탬피드**라고 한다. 예를 들어 상품 1,000개의 캐시가 정각에 모두 만료된다면, 그 순간 1,000건의 DB 쿼리가 동시에 발생해 데이터베이스가 일시적으로 과부하 상태에 빠진다. 지터는 만료 시각을 무작위로 분산시켜 이 충격을 완화한다.
