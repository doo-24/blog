...
서버의 성능은 결국 **동시에 몇 개의 요청을 처리할 수 있느냐**에 달린다. 이것을 결정하는 핵심이 I/O 모델과 동시성 전략이다.
...
대부분의 고성능 서버가 사용하는 것은 **I/O 멀티플렉싱 + Non-blocking I/O**다. "비동기"라고 부르지만, 엄밀히는 **동기 Non-blocking + 이벤트 통지**에 가깝다.
...
I/O 멀티플렉싱을 구조화한 것이 리액터 패턴이다. Node.js, Nginx, Redis, Netty의 핵심 아키텍처다.
...
Nginx의 `worker_processes auto;`가 바로 이 구조다. CPU 코어 수만큼 워커 프로세스를 생성하고, 각 워커가 독립적인 이벤트 루프를 운영한다.
...
이 한 줄로 Tomcat이 가상 스레드를 사용한다. 요청마다 가상 스레드가 할당되고, JDBC 호출에서 블로킹되어도 OS 스레드를 낭비하지 않는다.
...
코루틴은 **중단 가능한 함수**다. 실행을 일시 중지하고 나중에 이어갈 수 있다.
